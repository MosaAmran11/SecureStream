...
def optimize_and_clean_df(df):
    ...
    df[['Destination Port', 'Protocol', 'Label']] = df[[
        'Destination Port', 'Protocol', 'Label']].astype('category')
...
def reduce_mem_usage(props):
    ...
    for col in props.columns:
        # Exclude strings and datetime
        if props[col].dtype != object and props[col].dtype != 'datetime64[ns]':
